...
class MediaAssests(models.Model):
    ...
    #object attributes / table fileds
    ...
    # relationship attribute # object relationship - 'has a' : one user can have may uploads(one to many)
    uploaded_by = models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.CASCADE, related_name = 'media_assests')
    created_at = models.DateTimeField(auto_now_add=True) # automatically captures the time when a record is inserted
    updated_at = models.DateTimeField(auto_now_add=True)
    is_public = models.BooleanField(default=True)
    views_count = models.IntegerField(default=0)
    #always display my object in the order of creation
    class Meta:
        ordering = ['-created_at']
    #editing rights
